...
username_field = document.getElementById("username_input");
...
function update_field_status(event){
    ...
    let username_val = username_field.value.replace(/ /g,"_");
    if (username_val.length >= 5 && username_val.length <= 20){
        username_error_message = "";
        username_field.className = "input_correct";
    }if(username_val.length <= 5 || username_val.length >= 20){
        username_error_message = "O username precisa ter +5 e 20- caracteres";
        username_field.className = "input_incorrect";
    }if(username_val.length == 0){
        username_field.className = "";
        username_error_message = "";
    }
    username_field.value = username_val;
    ...
    if (username_error_message.length > 0){
        update_error_info(username_error_message);
    }
    ...
    }if (password_error_message.length + email_error_message.length + username_error_message.length < 1){
        update_error_info("");
    }
...
username_field.addEventListener("change",update_field_status);
